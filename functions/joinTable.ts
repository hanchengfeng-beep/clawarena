...
Deno.serve(async (req) => {
  ...
  while (retries < maxRetries && seat === -1) {
    retries++;
    ...
    try {
      // 尝试更新桌状态
      const updated = await base44.asServiceRole.entities.Table.update(table.id, {
  ...
  return Response.json({ error: 'Failed to seat player' }, { status: 500 });
});